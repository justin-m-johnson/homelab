...
resource "proxmox_vm_qemu" "staging1_Docker" {
    ...
    vmid = "900"
    name = "staging1.home.initcyber.net"
    desc = "staging 1 - Docker Container"
...
###########Start Ansible Provisioner########################
    ...
    provisioner "local-exec" {
        ...
        command = "ansible-playbook -u ${var.user} --key-file ${var.ssh_keys["priv"]} -i 172.16.10.50, MOTD.yml"
    }  
    provisioner "local-exec" {
        working_dir = "../../../2-Ansible-Provision/bash-shell/"
        command = "ansible-playbook -u ${var.user} --key-file ${var.ssh_keys["priv"]} -i 172.16.10.50, bash-shell.yml"
    }  
    provisioner "local-exec" {
        working_dir = "../../../2-Ansible-Provision/set-time-zone/"
        command = "ansible-playbook -u ${var.user} --key-file ${var.ssh_keys["priv"]} -i 172.16.10.50, set-time-zone.yml"
    }  
    provisioner "local-exec" {
        working_dir = "../../../2-Ansible-Provision/Docker-Install/"
        command = "ansible-playbook -u ${var.user} --key-file ${var.ssh_keys["priv"]} -i 172.16.10.50, Docker-Install-AMD.yml"
    }  
}